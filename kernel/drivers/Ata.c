...
void ide_read_buffer(unsigned char channel, unsigned char reg, unsigned int buffer,
   ...
   if (reg < 0x08)
      port_long_ins((uint16_t)channels[channel].base  + reg - 0x00, (uint32_t*)buffer, (uint32_t)quads);
   else if (reg < 0x0C)
      port_long_ins((uint16_t)channels[channel].base  + reg - 0x06, (uint32_t*)buffer, (uint32_t)quads);
   else if (reg < 0x0E)
      port_long_ins((uint16_t)channels[channel].ctrl  + reg - 0x0A, (uint32_t*)buffer, (uint32_t)quads);
   else if (reg < 0x16)
      port_long_ins((uint16_t)channels[channel].bmide + reg - 0x0E, (uint32_t*)buffer, (uint32_t)quads);
   //asm("popw %es;");
   if (reg > 0x07 && reg < 0x0C)
      ide_write(channel, ATA_REG_CONTROL, channels[channel].nIEN);
}
   ...
   for (int i = 0; i < 4; i++)
      if (ide_devices[i].Reserved == 1) {
	   ...
	   if (ide_devices[i].Size == 0) {
		kprint("empty");
           } else if (ide_devices[i].Size / 1024 / 1024 / 2 == 0) { // if not measurable in GB, print sectors
                kprint_int(ide_devices[i].Size);
		kprint(" sectors");
	   } else {
           	kprint_int(ide_devices[i].Size / 1024 / 1024 / 2);
           	kprint(" GB");
	   }
